...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (sessionStorage.getItem("introShown") === "true") {
    // Skip intro completely (no flash)
    if (intro) {
      intro.style.display = "none";
      intro.style.transform = "translateY(-100%)"; // keep it off-screen
    }
    if (mainContent) mainContent.style.display = "block";
    return; // ✅ stop here, don't attach START logic
  }

  // First load → show intro, hide main
  if (intro) {
  ...
  // START button → animate away intro
  if (startBtn && intro && mainContent) {
    startBtn.addEventListener("click", () => {
      intro.style.transform = "translateY(-100%)";
      setTimeout(() => {
        intro.style.display = "none";
        mainContent.style.display = "block";
        // ✅ Mark intro as shown only after START is clicked
        sessionStorage.setItem("introShown", "true");
      }, 1200); // match your CSS transition
    });
  }
});
...
const translations = {
  ...
  en: {
    ...
    Sauto2 : 'Automated systems require consistent care to remain efficient and safe. We provide specialized maintenance services that keep machines and robots performing at their best.',
...
// Attach listeners once on page load
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.toggle-btn').forEach(btn => {
    ...
    target.addEventListener('shown.bs.collapse', () => {
      btn.textContent = translations[CurrentLang].ShowLessBtn;
    });

    target.addEventListener("hidden.bs.collapse", () => {
      btn.textContent = translations[CurrentLang].ShowMoreBtn;
    });
  });
});
// Attach listeners once on page load
...
function applyTranslations() {
  ...
  // ✅ Initialize all collapse toggle buttons correctly
  document.querySelectorAll('.toggle-btn').forEach(btn => {
    const targetId = btn.getAttribute("data-bs-target");
    const target = document.querySelector(targetId);
    ...
    btn.textContent = target.classList.contains("show")
      ? translations[CurrentLang].ShowLessBtn
      : translations[CurrentLang].ShowMoreBtn;
  });
  // ✅ Initialize all collapse toggle buttons correctly
...
// Initialize all toggle buttons correctly
