...
function applyTranslations() {
  ...
  // ✅ Initialize all collapse toggle buttons to "More..."
  document.querySelectorAll('.toggle-btn').forEach(btn => {
    btn.textContent = translations[CurrentLang].ToggleBtnMore;
  });
...
document.querySelectorAll('.toggle-btn').forEach((button) => {
  ...
  // When the collapse is shown → switch to "Less"
  target.addEventListener('shown.bs.collapse', () => {
    button.textContent = translations[CurrentLang].ToggleBtnLess;
  });

  // When the collapse is hidden → switch to "More..."
  target.addEventListener("hidden.bs.collapse", () => {
    button.textContent = translations[CurrentLang].ToggleBtnMore;
  });
});
// services page toggle
