...
// End intro animation and show main content
...
//   const navbar = document.querySelector('.navbar');
//   if (window.scrollY > 50) {
//     navbar.style.background = getComputedStyle(document.body).getPropertyValue('--bg-main');
//   } else {
//     navbar.style.background = getComputedStyle(document.body).getPropertyValue('--bg-glass');
//   }
// });
...
// End navbar background change on scroll
 ...
 // Add the .show class dynamically
  // document.addEventListener('DOMContentLoaded', () => {
  //   const div = document.getElementById('myDiv');
  //   setTimeout(() => {
  //     div.classList.add('show'); // triggers fade-in
  //   }, 550);
  // });
// End fade-in effect
...
//   video.querySelector('source').src = '/Img/hero-dark.mp4';
// } else {
//   video.querySelector('source').src = '/Img/hero-light.mp4';
// }
...
function updateVideoSource() {
  const videoSrc = getComputedStyle(document.body).getPropertyValue('--video-src').trim();
  source.src = videoSrc.replace(/['"]/g, ""); // remove quotes if any
...
document.getElementById('lang-toggle').addEventListener('click', ()=>{
  ...
  /* The above code is setting the direction of the text on the webpage based on the value of the
  variable `currentLang`. If `currentLang` is equal to "ar" (Arabic), then the direction of the text
  will be set to right-to-left (rtl). Otherwise, the direction will be set to left-to-right (ltr). */
